...
public class YourService extends KiboRpcService {
    ...
    public void moveAndShot(int from, int to, int numberOfPhotos) {
        ...
        Log.i(TAG,
                "-------------- DEBUG: Turbulence(X,Y,Z)=(" + TurbX + "," + TurbY + "," + TurbZ + ") for point" + to);
    ...
    public double[] getTargetCenter(Mat list_ids, List<Mat> corners, Mat image) {
        ...
        //// (double)gray.rows()/16, // change this value to detect circles with
        // different distances to each other
        //// 100.0, 30.0, 1, 30); // change the last two parameters
        // HoughCircles(gray, circles, HOUGH_GRADIENT, 1.0, 30);
        // // (min_radius & max_radius) to detect larger circles
        // for (int x = 0; x < circles.cols(); x++) {
        // double[] center_array = circles.get(0, x); // 0: center_x, 1: center_y, 2:
        // radius
        // org.opencv.core.Point center = new
        // org.opencv.core.Point(Math.round(center_array[0]),
        // Math.round(center_array[1]));
        //
        // // HoughCirclesで検知した円の中心（center）が領域内に存在するか判定
        // double result = pointPolygonTest(new MatOfPoint2f(detect_area), center,
        // false);
        //
        // if (result > 0){
        // // circle center
